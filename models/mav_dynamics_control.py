...
class MavDynamics(MavDynamicsForces):
    ...
    def update(self, delta, wind, mode="all") -> list | None:
        ...
        mode can be "all" or "long" at this point
        """
    ...
    def _forces_moments(self, delta):
        ...
        CL = (1 - sigmoid) * (MAV.C_L_0 + MAV.C_L_alpha * self._alpha) + sigmoid * (
        ...
        F_lift = (
            ...
            * (
                CL
                + (MAV.C_L_alpha * self._alpha)
                + (MAV.C_L_q * MAV.c * q) / (2 * self._Va)
        ...
        C_X_delta_e = -MAV.C_D_delta_e * np.cos(self._alpha) + MAV.C_L_delta_e * np.sin(
            self._alpha
        )
        C_Z = -CD * np.sin(self._alpha) - CL * np.cos(self._alpha)
        C_Z_q = -MAV.C_D_q * np.sin(self._alpha) - MAV.C_L_q * np.cos(self._alpha)
        C_Z_delta_e = -MAV.C_D_delta_e * np.sin(self._alpha) - MAV.C_L_delta_e * np.cos(
            self._alpha
        )
        ...
        fx = (
            ...
            * (
                C_X
                + (C_X_q * MAV.c * q) / (2 * self._Va)
                + C_X_delta_e * delta.elevator
            )
            + thrust_prop
        )
        fz = fg_b[2] + q_bar * MAV.S_wing * (
            C_Z + (C_Z_q * MAV.c * q) / (2 * self._Va) + C_Z_delta_e * delta.elevator
        )
        fy = fg_b[1] + q_bar * MAV.S_wing * (
            MAV.C_Y_0
    ...
    def _motor_thrust_torque(self, Va, delta_t):
        ...
        thrust_prop = (
            0.5 * MAV.rho * MAV.S_prop * ((MAV.k_motor * delta_t) ** 2 - Va**2)
        )
        torque_prop = 0
